...
redirect such as a provider switcher cannot move the request), no MCP servers, no skills and no
saved session. Astra's tools are described in the system prompt and requested through validated
JSON structured output, so Astra keeps running its own tools and approvals.
"""
...
# The structured answer is one turn; a failed schema validation may take one more.
MAX_TURNS = 3
...
def system_prompt(messages: list[dict], tools: list[dict] | None, tool_choice=None) -> str:
    ...
    if tools:
        described = [{"name": t["function"]["name"], "description": t["function"].get("description", ""),
                      "parameters": t["function"].get("parameters") or {"type": "object"}}
                     for t in tools if isinstance(t, dict) and isinstance(t.get("function"), dict)]
        forced = tool_choice.get("function", {}).get("name") if isinstance(tool_choice, dict) else None
        sections.append(
            "# Tool protocol\n"
            "You cannot run tools yourself. Astra runs them for you and shows each result in the "
            "transcript. Answer with the required JSON object: `content` is your message to the user "
            "(it may be empty while you call tools) and `tool_calls` lists the calls to make now, each "
            "with the tool `name` and `arguments`, an object matching that tool's parameters. Use an "
            "empty `tool_calls` list when no tool is needed."
...
def answer_schema(tools: list[dict]) -> dict:
    names = sorted({t["function"]["name"] for t in tools if isinstance(t, dict) and isinstance(t.get("function"), dict)})
    return {"type": "object", "additionalProperties": False, "required": ["content", "tool_calls"],
            "properties": {"content": {"type": "string"}, "tool_calls": {"type": "array", "items": {
...
def _failure(result: dict) -> ClaudeCodeError:
    ...
    if result.get("subtype") in {"error_max_turns", "error_max_structured_output_retries"}:
        return ClaudeCodeError("Claude did not produce a valid structured answer; retry the request.")
...
class ClaudeCodeProvider:
    ...
    def _arguments(self, command: str, prompt_file: str, schema: dict | None) -> list[str]:
        model = (self.config.model or "sonnet").strip()
        arguments = [command, "-p", "--output-format", "stream-json", "--verbose", "--input-format", "stream-json",
                     "--model", model, "--system-prompt-file", prompt_file, "--restricted", "--tools", "",
    ...
    async def chat_stream(self, messages, tools=None, tool_choice=None, *, omit_tool_choice=False,
        ...
        tools = [t for t in tools or [] if isinstance(t, dict) and isinstance(t.get("function"), dict)]
        names = frozenset(t["function"].get("name", "") for t in tools)
        prepared = _messages_for_capabilities(messages, self.config.capabilities, names, self.config.vision_detail)
        schema = answer_schema(tools) if tools else None
        ...
        with tempfile.TemporaryDirectory(prefix="astra-claude-code-") as workdir:
            prompt_file = os.path.join(workdir, "system.md")
            Path(prompt_file).write_text(
                system_prompt(prepared, tools, None if omit_tool_choice else tool_choice) or "You are a helpful assistant.",
                encoding="utf-8")
            process = await asyncio.create_subprocess_exec(
                *self._arguments(command, prompt_file, schema), cwd=workdir, env=child_environment(),
            ...
            try:
                ...
                process.stdin.close()
                async for event in self._events(process, schema is not None):
                    yield event
            finally:
                _terminate(process)
                await process.wait()

    async def _events(self, process, structured: bool) -> AsyncGenerator[dict, None]:
        ...
        while True:
            ...
            if event.get("type") == "assistant":
                ...
                yield self._final(event, structured, reasoning)
                return

    def _final(self, result: dict, structured: bool, reasoning: str) -> dict:
        ...
        if not structured:
            ...
            if not content.strip():
                raise LLMResponseError("empty_response", "Claude Code returned an empty answer.")
            return {"type": "done", "content": content, "reasoning_content": reasoning,
                    "finish_reason": "stop", "usage": usage}
        answer = result.get("structured_output")
        if not isinstance(answer, dict) or not isinstance(answer.get("tool_calls"), list):
            raise LLMResponseError("incomplete_response", "Claude Code returned no structured answer.")
        calls = []
        for call in answer["tool_calls"]:
            ...
            calls.append({"id": f"call_{uuid.uuid4().hex[:24]}", "name": call["name"],
        ...
        return {"type": "tool_calls" if calls else "done", "calls": calls, "content": content,
                ...
                "tool_call_state": "complete", "usage": usage}
