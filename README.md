...
2. **Inside Astra**, enter `/connect`, select **Claude / Claude Code → Subscription**,
   then choose `fable`, `opus`, `sonnet` or `haiku` in `/model`.

Claude calls Astra's tools natively through a local bridge that never runs them; Astra runs
them with its usual approvals. The CLI runs without its own tools, settings files or saved sessions,
