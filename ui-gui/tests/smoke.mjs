...
try {
 ...
 // Command help operates on the local draft; completion never sends a command.
 const composerInput=page.getByRole('textbox',{name:'消息'});
 const suggestions=page.getByRole('listbox',{name:'指令建议'});
 const compactHeight=await page.locator('.composer').evaluate(el=>el.getBoundingClientRect().height);
 assert.ok(compactHeight<105,`empty composer should be compact, got ${compactHeight}`);
 await composerInput.fill('/'); await suggestions.getByRole('option').first().waitFor();
 const initialSuggestion=await composerInput.getAttribute('aria-activedescendant');
 await composerInput.press('ArrowDown'); assert.notEqual(await composerInput.getAttribute('aria-activedescendant'),initialSuggestion);
 await composerInput.press('ArrowUp'); assert.equal(await composerInput.getAttribute('aria-activedescendant'),initialSuggestion);
 await page.screenshot({path:join(output,'command-help.png')});
 await composerInput.fill('/模型'); await suggestions.getByRole('option').filter({hasText:'/model'}).click();
 assert.equal(await composerInput.inputValue(),'/model'); assert.equal(await suggestions.count(),0);
 assert.equal(await composerInput.evaluate(el=>el===document.activeElement),true);
 await composerInput.fill('/bar'); await composerInput.press('Enter');
 assert.equal(await composerInput.inputValue(),'/bar ');
 await suggestions.getByRole('option').filter({hasText:'/bar leave'}).click();
 assert.equal(await composerInput.inputValue(),'/bar leave');
 assert.equal((await page.evaluate(()=>window.astra.bootstrap())).sessions.length,0);
 assert.equal(requests.length,0);
 await composerInput.fill('/bar output ');
 assert.equal(await suggestions.getByRole('option').count(),3);
 await composerInput.press('Tab'); assert.equal(await composerInput.inputValue(),'/bar output atomic');
 assert.equal(await suggestions.count(),0);
 await composerInput.fill('/bar'); await composerInput.press('Escape');
 assert.equal(await composerInput.inputValue(),'/bar'); assert.equal(await suggestions.count(),0);
 await composerInput.fill('/min');
 await composerInput.evaluate(el=>el.dispatchEvent(new KeyboardEvent('keydown',{key:'Enter',code:'Enter',isComposing:true,bubbles:true,cancelable:true})));
 assert.equal(await composerInput.inputValue(),'/min');
 await composerInput.press('Shift+Enter'); assert.equal(await suggestions.count(),0);
 await composerInput.fill(Array.from({length:20},(_,i)=>`草稿第 ${i} 行`).join('\n'));
 assert.ok(await composerInput.evaluate(el=>el.scrollHeight>el.clientHeight&&el.clientHeight<=220));
 await composerInput.fill(''); assert.ok(await page.locator('.composer').evaluate(el=>el.getBoundingClientRect().height)<105);
 passed('compact composer grows and shrinks; slash matching, keyboard, pointer and IME never dispatch during completion');
 ...
 await delegateSummary.filter({hasText:'3 项进行中'}).waitFor(); assert.match(await delegateSummary.innerText(),/3 项进行中/);
 ...
 const beforeModeHelp=requests.length;
 await submit('/bar new'); await page.waitForFunction(()=>document.querySelector('.mode-badge')?.textContent==='bar');
 ...
 await page.screenshot({path:join(output,'command-help-bar-1000.png')});
 await page.evaluate(()=>document.documentElement.dataset.theme='dark');
 await page.screenshot({path:join(output,'command-help-dark.png')});
 await page.evaluate(()=>document.documentElement.dataset.theme='system');
 await app.evaluate(({BrowserWindow})=>BrowserWindow.getAllWindows()[0].setSize(1320,768));
 await composerInput.fill('/bar sessions'); await composerInput.press('Enter');
 await page.getByRole('dialog',{name:'管理会话'}).waitFor();
 assert.equal(await page.getByLabel('筛选对话模式',{exact:true}).inputValue(),'bar');
 assert.equal((await page.evaluate(()=>window.astra.query('sessions'))).some(entry=>entry.name==='sessions'),false);
 await page.getByRole('button',{name:'关闭',exact:true}).click();
 await composerInput.fill('/bar leave'); await composerInput.press('Enter');
 await page.waitForFunction(()=>document.querySelector('.mode-badge')?.textContent==='');
 await submit('/minimal new'); await page.waitForFunction(()=>document.querySelector('.mode-badge')?.textContent==='minimal');
 await composerInput.fill('保留的极简草稿');
 await page.getByRole('button',{name:'按需审批',exact:true}).click();
 await page.getByRole('button',{name:'查看极简模式指令',exact:false}).click();
 assert.equal(await page.getByRole('combobox',{name:'搜索命令与功能'}).inputValue(),'/minimal');
 await page.getByRole('button',{name:'关闭',exact:true}).click();
 assert.equal(await composerInput.inputValue(),'保留的极简草稿');
 await submit('/minimal sessions'); await page.getByRole('dialog',{name:'管理会话'}).waitFor();
 assert.equal(await page.getByLabel('筛选对话模式',{exact:true}).inputValue(),'minimal');
 await page.getByRole('button',{name:'关闭',exact:true}).click();
 await submit('/minimal leave'); await page.waitForFunction(()=>document.querySelector('.mode-badge')?.textContent==='');
 assert.equal(requests.length,beforeModeHelp);
 passed('mode-aware help preserves drafts, explains isolation and opens Bar/Minimal history without model calls or bogus sessions');
